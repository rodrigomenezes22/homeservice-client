...
function ManageProperties({userid}) {
    ...
    const getProperties = async () => {
      ...
      const handleDelete = async (propertyidentity) => {
        ...
        try {
            const res = await fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/api/property/${propertyidentity}`, {
              method: "DELETE",
              headers: { jwtToken: localStorage.jwtToken }
            });
  ...
  return (
    ...
    <div className="buttons-card">
      ...
      <button className="card-button button-primary" onClick={()=>handleDelete(property?.propertyid)}>
        <span class="material-symbols-rounded">
        delete
        </span>
        <p>Delete</p>
